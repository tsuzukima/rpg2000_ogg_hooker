...
`RPG2000_OGG_HOOKER_DECODE_CHAIN` env var として hook DLL に伝わります。
...
これらの flag は test 用途であり、production 配布では指定しないでください。
複数指定された場合は最後の指定が勝ちます。

`--stb-only` での実機テスト用の bat は [tools/test_stb_only.bat](tools/test_stb_only.bat) です。
`RPG2000.exe` と同じフォルダにコピーしてから実行してください。
